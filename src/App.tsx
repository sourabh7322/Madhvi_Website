    ...
    <>
      <nav className="fixed top-0 inset-x-0 z-10 px-5 sm:px-8 py-4 sm:py-5 flex flex-row justify-between items-center">
        <div className="flex flex-row gap-3 items-center">
          ...
          <span className="text-[25px] sm:text-[30px] text-white select-none tracking-[-0.02em]">✳︎</span>
        </div>
      ...
      <div className={`fixed inset-0 bg-black/95 backdrop-blur-sm z-[9] flex flex-col justify-center px-8 gap-8 transition-opacity duration-300 md:hidden ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        {links.map(link => (
          <a key={link} href="#" className="text-[32px] font-medium text-white hover:opacity-60 transition-opacity">{link}</a>
        ))}
...
function App() {
  ...
  // Seek frame-by-frame (reliable in all browsers) and store as ImageBitmap.
  // After load: scrubbing = drawImage only — zero seek, zero decode lag.
  useEffect(() => {
    const CAPTURE_W = 960;
    const CAPTURE_H = 540;
    const FPS = 18;
    ...
    video.muted = true;
    video.preload = 'auto';
    // Hide it but keep in DOM so browser processes frames
    video.style.cssText = 'position:fixed;top:-9999px;left:-9999px;width:1px;height:1px;opacity:0;pointer-events:none;';
    document.body.appendChild(video);

    const offscreen = new OffscreenCanvas(CAPTURE_W, CAPTURE_H);
    ...
    const extractFrames = async () => {
      ...
      // Clean up hidden video from DOM
  ...
  useEffect(() => {
    ...
    const handleMove = (clientX: number) => {
      ...
      prevXRef.current = clientX;
      const frameOffset = (delta / window.innerWidth) * SENSITIVITY * frames.length;

      targetFrameRef.current = Math.max(
        0,
        Math.min(frames.length - 1, targetFrameRef.current + frameOffset)
      );
    };

    const onMouseMove = (e: MouseEvent) => handleMove(e.clientX);
    const onTouchMove = (e: TouchEvent) => { if (e.touches.length) handleMove(e.touches[0].clientX); };
    const onTouchStart = (e: TouchEvent) => { if (e.touches.length) prevXRef.current = e.touches[0].clientX; };
    ...
    const render = () => {
      ...
      if (frames.length && canvas) {
        ...
        const idx = Math.max(0, Math.min(frames.length - 1, Math.round(currentFrameRef.current)));
        const c2d = canvas.getContext('2d')!;
        // GPU blit — instant, no decode, no seek
        c2d.drawImage(frames[idx], 0, 0, canvas.width, canvas.height);
      }
  ...
  return (
    <div className="relative min-h-screen bg-[#E5E5E5] w-full selection:bg-[#ff4b4b] selection:text-white">
      ...
      <div
        ...
        style={{ opacity: framesReady ? 0 : 1, pointerEvents: framesReady ? 'none' : 'all' }}
      >
        <div className="text-center">
          <div className="w-52 h-[2px] bg-white/15 rounded-full overflow-hidden mx-auto">
            <div
              className="h-full bg-white rounded-full transition-all duration-200 ease-out"
              style={{ width: `${loadProgress * 100}%` }}
            />
          </div>
          <p className="mt-4 text-[13px] tracking-widest uppercase text-white/30">Loading</p>
        </div>
      </div>

      {/* Canvas background — replaces video element */}
      <canvas
        ref={canvasRef}
        width={960}
        height={540}
